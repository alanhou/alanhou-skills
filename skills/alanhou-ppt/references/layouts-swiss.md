...
1. 同时打开三份页面:原始参考 PPT、当前 `template-swiss.html` 或生成页、正在修改的测试 PPT。原始参考路径是 `assets/template-swiss.html`。
...
**注意**:
- 图片优先用 `images/{页号}-{语义}.png` 本地文件(用户提供素材、`scripts/fetch-stock-image.mjs` 图库下载,或 GPT-M 2.0 生成),不要外链 unsplash 等图床——图库照片也必须先落盘进 `images/` 再引用
