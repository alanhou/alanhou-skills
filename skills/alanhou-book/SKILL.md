---
name: alanhou-book
description: 拆一本书，以「问题」为轴心走一条线。五件事：作者在答什么问题（问题），这个问题之前各流派/社会共识怎么答（零点），作者带来什么独特洞见——公式/理论框架/模型/概念四选一——相对共识挪动了什么（位移/delta），落成哪句结论（落点），最后萃一个 takeaway 作为精神内核（行囊）。收尾画一张 ASCII 参考系图（千脑智能式）：各流派、旧共识、作者钉到同一张图的位置上，delta 是图上一段看得见的距离，再走两步做预测——看懂这本书在认知史里挪动了哪一步，还能拿它预测书外的新事。Accepts a book title (web-grounded) or a local book file (EPUB / PDF / TXT, full-text grounded). Use when user says '拆书', '拆这本', '分析这本书', '这本书在讲什么', '压缩一本书', 'book', or shares a book name / book file wanting structural analysis. NOT FOR 章节摘要、论文（用 alanhou-paper）、单一观点深钻（用 alanhou-writes）.
user_invocable: true
---
...
掌握不掌握就看这步。在图上挪到一个作者没去过的位置——最好挨着用户自己的世界——用作者的框预测：站这个新位置上，他会说什么？预测完回头对账。猜对了，图就是你的了。
...
- Markdown 文件里图包在三反引号代码块中（保等宽，渲染不碎）
- 宽度 ≤80 字符
- 轴有名字，两端有极
...
如果用户给了本地书文件——以全文为准，不用查网：
- **EPUB**：`python3 scripts/extract-epub.py <book.epub> /tmp/book.txt` 提取全文（纯标准库，无依赖）
- **PDF**：Claude Code 直接 Read；其他环境用 `pdftotext <book.pdf> /tmp/book.txt`
- **TXT / Markdown**：直接读
- 长书别从头读到尾：先读目录 + 导言 + 结论（问题和落点多半在这），再按目录挑承重章节读。引用术语/金句必须出自原文，页码/章节注明
- grep 关键词定位：`grep -n "关键词" /tmp/book.txt` 比通读快

如果用户给了 PDF 链接 / arxiv / 粘贴内容——直接读，不用查。
...
1. 日期：`date "+%Y-%m-%d %H:%M"`
2. 写入 `~/Documents/notes/{YYYYMMDD}-拆书-{书名}.md`（用户指定位置则优先）
3. 标准 Markdown：标题从 `##` 开始，加粗 `**bold**`
4. 文件头（YAML frontmatter）：
...
5. 正文六段：1-5 抽骨架（散文，冷），第 6 段「认知地图」（ASCII 参考系图 + 走两步预测，热）。ASCII 图包在三反引号代码块中。
6. 报告文件路径给用户。
7. 顺手问一句要不要衍生品：做成卡片（alanhou-card，认知地图很适合 `-w` 白板或 `-i` 信息图）、分享 deck（alanhou-ppt）、短视频（alanhou-video）。不要默认生成。
