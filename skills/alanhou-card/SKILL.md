...
<!-- attribution: alanhou-card is derived from ljg-card in ljg-skills by lijigang (https://github.com/lijigang/ljg-skills), MIT. -->
...
本 skill 输出为视觉文件（PNG），不受纯文本/ASCII 类全局规范约束。

## 共享基础

### 获取内容

- URL --> 抓取网页内容（Claude Code 用 WebFetch；Codex / Gemini CLI 用内建 fetch 工具或 curl）
- 粘贴文本 --> 直接使用
- 文件路径 --> Read 获取
- 视频链接（YouTube / B站 / 小红书 / 抖音等）--> 用 alanhou-video-fetch skill 取清洗后的文字稿作为内容；若该 skill 不可用，直接运行 `bash ~/.agents/skills/alanhou-video-fetch/scripts/fetch-video.sh "<URL>"`（B站 AI 字幕、小红书需加 `-c chrome`）
