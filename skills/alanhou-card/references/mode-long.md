...
数一下正文字数。> 约 800 字（或明显超过 3 屏）时停下：除非用户明确坚持单张长图，建议改走 `-m` 多卡（`references/mode-poster.md`）。超长长图加载慢、转发糊、没人滑到底。
...
- 按空行分割为段落列表
- **不做切分**：所有内容放在一张卡内（前提是过了步骤 0 的长度检查）
...
写入：`/tmp/alanhou_cast_long_{name}.html`

## 步骤 5：截图

```bash
node assets/capture.js /tmp/alanhou_cast_long_{name}.html ~/Downloads/{name}.png 1080 800 fullpage
```
