...
# alanhou-paper: 把一篇论文当一个故事讲

<!-- attribution: alanhou-paper is derived from ljg-paper in ljg-skills by lijigang (https://github.com/lijigang/ljg-skills), MIT. -->
...
- title 与文件名：文件名 `paper-{简短标题}` 的「简短标题」是方法名 / 核心概念名（如 `prerl`、`dsrl`），用于文件检索；title 是故事内核句，用于内容入口——两者不同字段，互不替代。
...
10. *6 个月后的我看得懂吗？* — 每个术语首次出现必须落地（"value function = 给定当前状态预测未来累积奖励"），每个公式必须翻译成自然语言，每个引用都要说明对外行的意义。自检：默想"半年后我在笔记库里搜到这篇，30 秒内能回想起核心吗？"——不能 → 重写
...
- arxiv URL → 网页抓取（WebFetch / fetch / curl，按环境选择）
...
如果论文有一张承载全文核心思路的总览图（overview / architecture diagram，通常是 Figure 1），提取并保存到 `~/Documents/notes/images/`，文件名 `{YYYYMMDD}-paper-{简短标题}-overview.png`。

判断标准：这张图让人一看就抓住论文在做什么。不是所有论文都有——没有就跳过，不要硬找。

提取方法：
- arxiv → 访问 HTML 版（`arxiv.org/html/...`），找到图片 URL，用网页抓取工具下载（WebFetch / curl，按环境选择）
- PDF → 截取含图页面保存为图片
...
读 `references/template.md`，按「文件规范」命名，写入 `~/Documents/notes/`（用户指定位置则优先）。
